...
def get_total_pages_com(url):
    source_code = get_source_code(url)
    soup = BeautifulSoup(source_code, 'lxml')
    total_pages = 0
    try:
        page_info = soup.find('h2',{'class':'total fl'})
    except AttributeError as e:
        page_info = None

    if page_info == None:
        return None
    page_info_str = int(page_info.find("span").get_text().strip())   #'<h2 class="total fl">共找到<span> 611 </span>个小区</h2>'

    total_pages = math.ceil((page_info_str / 30))
    if total_pages > 0:
        total_pages += 1
    return total_pages

def get_rent_total_pages(url):
    source_code = get_source_code(url)
    soup = BeautifulSoup(source_code, 'lxml')
    total_pages = 0
    try:
    ...
    if page_info == None:
        return None
    page_info_str = page_info.get('data-totalpage')  #'data-totalPage=100'
